...
func Getid(bot *gotgbot.Bot, context *ext.Context) error {
	...
	if len(context.Args()) == 2 {
		...
		_, err = Bot.SendMessage(context.Message.From.Id, fmt.Sprintf("Firstname: %v\nLastname: %v\nUsername: %v\nUserID: %v", target.FirstName, target.LastName, target.Username, target.Id), &gotgbot.SendMessageOpts{ParseMode: gotgbot.ParseModeHTML})
		return err
	}
	_, err := Bot.SendMessage(context.Message.From.Id, fmt.Sprintf("Firstname: %v\nLastname: %v\nUsername: %v\nUserID: %v", context.Message.From.FirstName, context.Message.From.LastName, context.Message.From.Username, context.Message.From.Id), &gotgbot.SendMessageOpts{ParseMode: gotgbot.ParseModeHTML})
	return err
}
