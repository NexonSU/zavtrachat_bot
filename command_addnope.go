...
func AddNope(bot *gotgbot.Bot, context *ext.Context) error {
	...
	if result.Error != nil {
		return Reply(fmt.Sprintf("Не удалось добавить nope, ошибка:\n<code>%v</code>", result.Error.Error()), *context)
	}
	return Reply(fmt.Sprintf("Nope добавлен как <code>%v</code>.", nope.Text), *context)
}
