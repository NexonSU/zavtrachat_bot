...
func Cur(bot *gotgbot.Bot, context *ext.Context) error {
	...
	_, err = context.Message.Reply(bot, fmt.Sprintf("%.2f %v = %.2f %v", conversion.Amount, conversion.Name, resultAmount, resultName), &gotgbot.SendMessageOpts{ParseMode: gotgbot.ParseModeHTML, ReplyParameters: &gotgbot.ReplyParameters{AllowSendingWithoutReply: true}})
	return err
}
