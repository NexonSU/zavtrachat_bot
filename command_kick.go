...
func Kick(bot *gotgbot.Bot, context *ext.Context) error {
	...
	return Reply(fmt.Sprintf("Пользователь <a href=\"tg://user?id=%v\">%v</a> исключен.", target.Id, UserFullName(&target)), *context)
}
