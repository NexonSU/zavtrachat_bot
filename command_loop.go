...
func Loop(bot *gotgbot.Bot, context *ext.Context) error {
	if context.Message.ReplyToMessage == nil {
		return Reply("Пример использования: <code>/loop</code> в ответ на какое-либо сообщение с видео.", *context)
	}

	if !IsContainsMedia(context.Message.ReplyToMessage) {
		return Reply("Какого-либо медиа нет в указанном сообщении.", *context)
	}
