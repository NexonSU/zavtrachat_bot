...
func Convert(bot *gotgbot.Bot, context *ext.Context) error {
	if context.Message.ReplyToMessage == nil {
		...
		return Reply("Пример использования: <code>/convert</code> в ответ на какое-либо сообщение с медиа-файлом.\nДопольнительные параметры: gif,mp3,ogg,jpg.", *context)
	}
	if !IsContainsMedia(context.Message.ReplyToMessage) {
		...
		return Reply("Какого-либо медиа файла нет в указанном сообщении.", *context)
	}
